...
class History {
  public:
    ...
    inline Move consult_counter(const Color &stm, const Move &past_move) const {
        // TODO try the usual indexing ([piece_type][to]), instead of butterfly
        if (past_move == MOVE_NONE)
            return MOVE_NONE;
        return m_counter_moves[stm][past_move.from_and_to()];
    }
    inline bool is_killer(const Move &move, const int &depth) const {
        return move == consult_killer1(depth) || move == consult_killer2(depth);
    }
    inline bool is_counter(const Color &stm, const Move &move, const Move &past_move) const {
        return move == consult_counter(stm, past_move);
    }
  ...
  private:
    ...
    inline void save_counter(const Color &stm, const Move &past_move, const Move &move) {
        if (past_move != MOVE_NONE)
            m_counter_moves[stm][past_move.from_and_to()] = move;
    }

    HistoryType m_capture_history[2][6][64][5];
    HistoryType m_search_history_table[COLOR_NB][64 * 64];
    Move m_counter_moves[2][64 * 64];
