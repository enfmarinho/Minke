...
bool under_attack(const Position &position, const Player &attacker_player, const PiecePlacement &pp_defender) {
    ...
    return cheapest_attacker(position, attacker_player, pp_defender, trash) != Piece::None;
}
...
MoveList::MoveList(const Position &position) { gen_pseudolegal_moves(position); }
...
void MoveList::gen_pseudolegal_moves(const Position &position) {
    for (IndexType file = 0; file < BoardHeight; ++file) {
        for (IndexType rank = 0; rank < BoardWidth; ++rank) {
            ...
            if (square.player == Player::None || position.side_to_move() != square.player) {
                continue;
            }
            switch (square.piece) {
                case Piece::Pawn:
...
[[nodiscard]] MoveList::size_type MoveList::size() const { return m_end - m_begin; }
...
void MoveList::pseudolegal_pawn_moves(const Position &position, const PiecePlacement &from) {
    ...
    if (!capture_left.out_of_bounds() && position.consult(capture_left).player == adversary) { // left capture
        if (capture_left.file() == promotion_file) {
            m_move_list[m_end++] = Move(from, capture_left, MoveType::PawnPromotionQueen);
            m_move_list[m_end++] = Move(from, capture_left, MoveType::PawnPromotionRook);
            m_move_list[m_end++] = Move(from, capture_left, MoveType::PawnPromotionBishop);
            m_move_list[m_end++] = Move(from, capture_left, MoveType::PawnPromotionKnight);
        } else {
            m_move_list[m_end++] = Move(from, capture_left, MoveType::Capture);
        }
    } else if (!capture_left.out_of_bounds() && position.en_passant_rank() == capture_left.rank() &&
    ...
    if (!capture_right.out_of_bounds() && position.consult(capture_right).player == adversary) { // right capture
        if (capture_right.file() == promotion_file) {
            m_move_list[m_end++] = Move(from, capture_right, MoveType::PawnPromotionQueen);
            m_move_list[m_end++] = Move(from, capture_right, MoveType::PawnPromotionRook);
            m_move_list[m_end++] = Move(from, capture_right, MoveType::PawnPromotionBishop);
            m_move_list[m_end++] = Move(from, capture_right, MoveType::PawnPromotionKnight);
        } else {
            m_move_list[m_end++] = Move(from, capture_right, MoveType::Capture);
        }
    } else if (!capture_right.out_of_bounds() && position.en_passant_rank() == capture_right.rank() &&
    ...
    if (position.consult(singlemove).piece == Piece::None) {
        if (singlemove.file() == promotion_file) {
            m_move_list[m_end++] = Move(from, singlemove, MoveType::PawnPromotionQueen);
            m_move_list[m_end++] = Move(from, singlemove, MoveType::PawnPromotionRook);
            m_move_list[m_end++] = Move(from, singlemove, MoveType::PawnPromotionBishop);
            m_move_list[m_end++] = Move(from, singlemove, MoveType::PawnPromotionKnight);
        } else {
            m_move_list[m_end++] = Move(from, singlemove, MoveType::Regular);
        }

        PiecePlacement doublemove(from.index() + 2 * offset);
        if (from.file() == original_file && position.consult(doublemove).piece == Piece::None) {
            m_move_list[m_end++] = Move(from, doublemove, MoveType::Regular);
        }
    }
}

void MoveList::pseudolegal_knight_moves(const Position &position, const PiecePlacement &from) {
...
void MoveList::pseudolegal_sliders_moves(const Position &position, const PiecePlacement &from) {
...
void MoveList::pseudolegal_king_moves(const Position &position, const PiecePlacement &from) {
...
void MoveList::pseudolegal_castling_moves(const Position &position) {
    ...
    if (castling_rights.king_side && position.consult(player_perspective_first_file, 5).piece == Piece::None &&
        position.consult(player_perspective_first_file, 6).piece == Piece::None &&
        position.consult(player_perspective_first_file, 7) == Square(Piece::Rook, position.side_to_move()) &&
        !under_attack(position, adversary, PiecePlacement(player_perspective_first_file, 5)) &&
        !under_attack(position, adversary, PiecePlacement(player_perspective_first_file, 6)))
        m_move_list[m_end++] =
            Move(king_placement, PiecePlacement(player_perspective_first_file, 6), MoveType::KingSideCastling);

    if (castling_rights.queen_side && position.consult(player_perspective_first_file, 1).piece == Piece::None &&
        ...
        position.consult(player_perspective_first_file, 0) == Square(Piece::Rook, position.side_to_move()) &&
        !under_attack(position, adversary, PiecePlacement(player_perspective_first_file, 2)) &&
        !under_attack(position, adversary, PiecePlacement(player_perspective_first_file, 3)))
        m_move_list[m_end++] =
            Move(king_placement, PiecePlacement(player_perspective_first_file, 2), MoveType::QueenSideCastling);
}
...
Piece cheapest_attacker(const Position &position, const Player &attacker_player, const PiecePlacement &pp_defender,
    ...
    auto under_pawn_thread = [&position, &attacker_player](const PiecePlacement &pawn_atacker_pos) -> bool {
    ...
    PiecePlacement east_pawn_attacker(pp_defender.index() - opponent_pawn_offset + offsets::East);
    if (under_pawn_thread(east_pawn_attacker))
        return pp_atacker = east_pawn_attacker, Piece::Pawn;

    PiecePlacement west_pawn_attacker(pp_defender.index() - opponent_pawn_offset + offsets::West);
    ...
    for (const IndexType &offset : offsets::Knight) {
        ...
        if (!to.out_of_bounds()) {
            ...
            if (to_square.player == attacker_player && to_square.piece == Piece::Knight)
    ...
    for (const IndexType &offset : offsets::Sliders[0]) { // Bishop directions
        ...
        for (PiecePlacement to(pp_defender.index() + offset); !to.out_of_bounds(); to.index() += offset) {
            const Square &to_square = position.consult(to);
...
bool SEE(const Position &position, const Move &move) {
    Player player = position.side_to_move(), opponent = player == Player::White ? Player::Black : Player::White;
    Position copy_position = position;
    WeightType material_gain = weights::SEE_table[piece_index(copy_position.consult(move.to).piece)];
    WeightType material_risk = weights::SEE_table[piece_index(copy_position.consult(move.from).piece)];
    copy_position.consult(move.from) = EmptySquare;

    while (material_gain <= material_risk) {
        PiecePlacement pp_atacker;
        Piece attacker = cheapest_attacker(copy_position, opponent, move.to, pp_atacker);
...
void MoveList::calculate_scores(const GameState &game_state, const Move &tt_move) {
    ...
    for (size_type index = 0, remaining = size(); index < remaining; ++index) {
        ...
        if (move == tt_move) {
            ...
            m_move_scores[index] = CaptureScore +
                                   5 * weights::SEE_table[piece_index(game_state.top().consult(move.to).piece)] -
                                   weights::SEE_table[piece_index(game_state.top().consult(move.from).piece)] / 5 -
