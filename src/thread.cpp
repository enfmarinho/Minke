...
bool Thread::should_stop() const {
    return m_stop || (!m_infinite && (m_nodes_searched >= m_node_limit || now() - m_start_time >= m_search_time));
}
...
void Thread::search(GameState &game_state) {
    if (m_stop) {
        m_stop = false;
        m_start_time = now();
        m_thread = std::thread(search::search, std::ref(game_state), std::ref(*this));
    }
}

CounterType Thread::max_depth_ply() const { return m_max_depth; }

void Thread::max_depth_ply(CounterType new_max_depth) { m_max_depth = new_max_depth; }

void Thread::node_limit(uint64_t new_node_limit) { m_node_limit = new_node_limit; }
...
void Thread::set_search_time(TimeType search_time) { m_search_time = search_time; }
