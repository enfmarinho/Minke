...
bool GameState::make_move(const Move &move) {
    ...
    if (move.move_type == MoveType::Regular) {
        ...
        const Square &captured = m_position_stack[m_position_stack.size() - 2].consult(move.to);
        m_net.remove_feature(captured, move.to);
    } else if (move.move_type == MoveType::EnPassant) {
        const IndexType pawn_offset = moved.player == Player::White ? offsets::North : offsets::South;
        const Player adversary = moved.player == Player::White ? Player::Black : Player::White;
        m_net.remove_feature({Piece::Pawn, adversary}, PiecePlacement(move.to.file() - pawn_offset, move.to.rank()));
    ...
    } else if (move.move_type == MoveType::PawnPromotionQueen || move.move_type == MoveType::PawnPromotionKnight ||
               move.move_type == MoveType::PawnPromotionRook || move.move_type == MoveType::PawnPromotionBishop) {
