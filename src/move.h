...
class Move {
  public:
    ...
    inline Move(int16_t bytes, PieceType piece) : m_bytes(bytes), m_piece(piece) {};
    inline Move(Square from, Square to, MoveType move_type, PieceType piece) {
        m_bytes = (move_type << 12) | (to << 6) | (from);
        m_piece = piece;
    }
...
class PackedMove {
  public:
    inline PackedMove() : m_bytes(0) {};
    inline PackedMove(int16_t bytes) : m_bytes(bytes) {};
    inline PackedMove(Move move) : m_bytes(move.internal()) {}
    inline PackedMove(Square from, Square to, MoveType move_type) { m_bytes = (move_type << 12) | (to << 6) | (from); }
...
const PackedMove PACKED_MOVE_NONE = PackedMove();
