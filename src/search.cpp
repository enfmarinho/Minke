...
ScoreType negamax(ScoreType alpha, ScoreType beta, CounterType depth, const bool cutnode, ThreadData &td) {
    ...
    if (in_check) {
        ...
        td.tt->save(position.get_hash(), 0, MOVE_NONE, SCORE_NONE, eval, BOUND_EMPTY, ttpv);
    }
    ...
    if (!in_check && !pv_node && !root && !singular_search) {
        ...
        if (depth >= probcut_min_depth() && std::abs(beta) < MATE_FOUND &&
            ...
            while ((move = move_picker.next_move(true)) != MOVE_NONE) {
                ...
                if (pc_score >= pc_beta) {
                    td.tt->save(position.get_hash(), depth - 3, move, pc_score, eval, LOWER, ttpv);
                    return pc_score;
                }
            }
        }
    ...
    if (!td.stop_search()) { // Save on TT if search was completed
        ...
        td.tt->save(position.get_hash(), depth, best_move, best_score, eval, bound, ttpv);
...
ScoreType quiescence(ScoreType alpha, ScoreType beta, ThreadData &td) {
    ...
    if (in_check) {
        ...
        td.tt->save(position.get_hash(), 0, MOVE_NONE, SCORE_NONE, static_eval, BOUND_EMPTY, ttpv);
    }
    ...
    td.tt->save(position.get_hash(), 0, best_move, best_score, static_eval, bound, ttpv);
