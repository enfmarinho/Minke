...
void History::update_history_heuristic_score(const Position &position, const Move &move, int bonus) {
    HistoryType *ptr = &m_search_history_table[position.get_stm()][move.from_and_to()][position.is_attacked(move.from())][position.is_attacked(move.to())];
    update_score(ptr, bonus);
}
...
HistoryType History::get_history_heuristic_score(const Position &position, const Move &move) const {
    return m_search_history_table[position.get_stm()][move.from_and_to()][position.is_attacked(move.from())][position.is_attacked(move.to())];
}
