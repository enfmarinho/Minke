...
ScoreType History::correct_eval(const Position &position, const ScoreType &raw_eval) const {
    ScoreType corrected_eval = raw_eval * (200 - position.get_fifty_move_ply()) / 200;
    corrected_eval += get_corr_history(position);

    return std::clamp(corrected_eval, static_cast<ScoreType>(-MATE_FOUND + 1), static_cast<ScoreType>(MATE_FOUND));
}
