...
void TimeManager::update(double node_fraction, CounterType pv_stability, CounterType score_stability) {
    ...
    const double node_spent_scale =
        std::max(tm_node_spent_base() / 1000.0 - node_fraction * tm_node_spent_factor() / 1000.0,
                 tm_node_spent_min_scale() / 1000.0);

    m_scale = pv_stability_scale * score_stability_scale * node_spent_scale;
}
