...
bool TranspositionTable::probe(const Position &position, TTEntry &tte) {
    ...
    KeyType target_key = key_from_hash(position.get_hash());
    for (TTEntry &entry : m_table[table_index].entry) {
        tte = entry;
        if (tte.key() == target_key)
            return true;
    }
    return false;
}

void TranspositionTable::save(const HashType &hash, const IndexType &depth, const Move &best_move,
    ...
    replace->store(hash, depth, best_move, score, eval, bound, was_pv);
}
