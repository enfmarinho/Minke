...
bool Position::set_fen(const std::string &fen) {
    ...
    for (char c : fen_arguments[0]) {
        ...
        if (!std::isdigit(c)) {
            ...
            if (piece_char == 'p')
                add_piece({get_piece(PAWN, player), sq});
            else if (piece_char == 'n')
                add_piece({get_piece(KNIGHT, player), sq});
            else if (piece_char == 'b')
                add_piece({get_piece(BISHOP, player), sq});
            else if (piece_char == 'r')
                add_piece({get_piece(ROOK, player), sq});
            else if (piece_char == 'q')
                add_piece({get_piece(QUEEN, player), sq});
            else if (piece_char == 'k')
                add_piece({get_piece(KING, player), sq});
            else
                assert(false);
...
void Position::reset_nnue() { m_nnue.init(*this); }
...
void Position::make_castle(const Move &move, DirtyPiece &dp) {
    ...
    dp.add0 = {piece, to};
    dp.add1 = {rook, NO_SQ}; // Square will be set be the switch
...
void Position::make_promotion(const Move &move, DirtyPiece &dp) {
    ...
    dp.add0 = {get_piece(move.promotee(), get_stm()), to};
...
void Position::unmake_move(const Move &move) {
    ...
    if (move.is_regular()) {
        ...
        switch (to) {
            case g1: // White castle short
                remove_piece({get_piece(ROOK, get_stm()), f1});
                add_piece({WHITE_ROOK, msb(rook_castle_bb)});
                break;
            case c1: // White castle long
                remove_piece({get_piece(ROOK, get_stm()), d1});
                add_piece({WHITE_ROOK, lsb(rook_castle_bb)});
                break;
            case g8: // Black castle short
                remove_piece({get_piece(ROOK, get_stm()), f8});
                add_piece({BLACK_ROOK, msb(rook_castle_bb)});
                break;
            case c8: // Black castle long
                remove_piece({get_piece(ROOK, get_stm()), d8});
                add_piece({BLACK_ROOK, lsb(rook_castle_bb)});
                break;
            default:
                __builtin_unreachable();
        }
    ...
    // TODO avoid unnecessary hashing
    if (m_curr_state.en_passant != NO_SQ)
        hash_ep_key();
    hash_castle_key();
...
bool Position::no_legal_moves() {
    ...
    for (ScoredMove *curr = moves; curr != end; ++curr) {
        DirtyPiece dp;
        bool legal = make_move(curr->move);
...
bool Position::is_legal(const Move &move) {
    ...
    if (moved_pt == KING) {
        remove_piece({get_piece(KING, m_stm), king_sq});
        bool is_king_attacked = is_attacked(to);
        add_piece({get_piece(KING, m_stm), king_sq});
        return !is_king_attacked;
    }
